...
class ioOperations {
    ...
    public function getByteArrayFromIVString() {
        $correct = false;
        $retv = "";

        $in = htmlspecialchars(trim($_POST['iv']));

        if ((!isset($in)) || ($in === NULL)) {
            $_SESSION['debug'] .= "iv not correct";
        } else if (empty($in)) {
            $_SESSION['debug'] .= "iv is empty";
        } else {
            //removing spaces and 0x hex format specifiers is not fool-proof but gets normal input to  hex format
            $in = str_replace(' ', '', $in); //remove all spaces from string
            $in = str_replace('\x', '', $in); // remove all 0x hex format specifiers
            ...
            $_SESSION['debug'] .= "The iv string converted to a byte-array with the decimal representation of the keybytes:";
            $_SESSION['debug'] .= "\n" . implode(",", $bytearray) . "\n";
            $retv = $bytearray;
            $correct = true;
        }
    ...
    public function getByteArrayFromInput() {
        ...
        } else if (!(($_POST['operation'] == 'subBytes') || ($_POST['operation'] == 'shiftRows') || ($_POST['operation'] == 'mixColumns') || ($_POST['operation'] == 'addRoundKey') || ($_POST['operation'] == 'encrypt') || ($_POST['operation'] == 'decrypt') )) {
