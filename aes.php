...
//end function decrypt
    ...
    public function invSubBytes($state) {
        for ($row = 0; $row < 4; $row++) { // for all 16 bytes in the (4x4-byte) State
            for ($column = 0; $column < 4; $column++) { // for all 16 bytes in the (4x4-byte) State
                $_SESSION['debug'] .= "state[$row][$column]=" . $state[$row][$column] .
                        "-->" . self::$sBoxInv[$state[$row][$column]] . "\n";
                $state[$row][$column] = self::$sBoxInv[$state[$row][$column]];
            }
        }
        return $state;
    }
    ...
    public function invShiftRows($state) {
        ...
        for ($row = 0; $row < 4; $row++) {
            for ($column = 0; $column < 4; $column++) {
                //shiftright the rows n positions for row n, so 0 for row 0, 1 position for row 1, etc.
